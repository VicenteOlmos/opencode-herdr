...
When the **opencode-herdr** plugin is installed, OpenCode also exposes slash commands (mechanical — no LLM turn):

- `/herdr-status` — Herdr availability, runtime discovery, and target count
- `/herdr-test` — smoke test (`herdr --version`, snapshot) and optional pane probe inside Herdr
- `/herdr-delete` — close all `oh-*` job panes in the dedicated `opencode-herdr` tab (does not uninstall the plugin)
- `/herdr-pane <runtime> <task>` — delegate a task through a Herdr runtime
- `/herdr-handover <runtime> [note]` — split a pane and send handover context
